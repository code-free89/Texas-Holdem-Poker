...
function handle_how_many_reply (opponents) {
  console.log('handle_how_many_reply');
  gui_write_modal_box("");
  new_game_continues(1);
...
function clear_player_cards (count) {
  console.log('clear_player_cards');
...
function new_game () {
  ...
  HUMAN_WINS_AGAIN = 0;
  initialize_game();
  // Neal Switch here - to avoid prompting 
  //ask_how_many_opponents();
  handle_how_many_reply(1); 
}

function new_game_continues (req_no_opponents) {
  console.log('new_game_continues');
 var my_players = [
                    new player("Opponent", 0, "", "", "", 0, 0),
  ...
  button_index = 1   // Neal - in our version of the game, opponent is always the dealer 
  new_round();
}

function number_of_active_players () {
  console.log('number_of_active_players')
  var num_playing = 0;
  var i;
  for (i = 0; i < players.length; i++) {
    if (has_money(i)) {
...
function blinds_and_deal () {
  ...
  if (num_playing == 3) {
    ...
    SMALL_BLIND = hands[handindex]['chipsInPot'] - hands[handindex]['computerBet'];
    BIG_BLIND = hands[handindex]['computerBet'];
  }
...
function deal_and_write_a () {
  ...
  players[0].carda = JSON.stringify(hands[handindex]['playerHoleCards'][0]);
  players[1].carda = JSON.stringify(hands[handindex]['computerHoleCards'][0]);
...
function deal_and_write_b () {
  ...
  players[0].cardb = JSON.stringify(hands[handindex]['playerHoleCards'][1]);
  players[1].cardb = JSON.stringify(hands[handindex]['computerHoleCards'][1]);
  current_player = get_next_player_position(button_index, 1);
  unroll_player(current_player, current_player, delay_for_main);
}

function go_to_betting () {
  console.log('go_to_betting');
...
function deal_flop () {
  ...
  gui_write_game_response(message);
  for (var i = 0; i < 3; i++) {
    board[i] = JSON.stringify(hands[handindex]['flopCards'][i]);
  }
...
function deal_fourth () {
  ...
  var burn = JSON.stringify(hands[handindex]['turnCard']);
  gui_burn_board_card(1, burn);
  var message = "<tr><td><font size=+2><b>Dealing turn</b></font>";
  gui_write_game_response(message);
  board[3] = JSON.stringify(hands[handindex]['turnCard']);
...
function deal_fifth () {
  ...
  var burn = JSON.stringify(hands[handindex]['riverCard']);
  gui_burn_board_card(2, burn);
  var message = "<tr><td><font size=+2><b>Dealing river</b></font>";
  gui_write_game_response(message);
  board[4] = JSON.stringify(hands[handindex]['riverCard']);
    ...
    if (current_bettor_index == 0) {
      ...
      var do_raise = function () {
        if(current_bet_amount == 0)
          handle_human_bet(BIG_BLIND);
        else
          handle_human_bet(current_bet_amount * 2);        
      }
...
function handle_end_of_round () {
  console.log('handle_end_of_round');
...
function autoplay_new_round () {
  console.log('autoplay_new_round');
  if (STOP_AUTOPLAY > 0) {
    STOP_AUTOPLAY = 0;
    new_game();
  } else {
...
function ready_for_next_card () {
  console.log('ready_for_next_card');
...
function make_readable_rank (r) {
  console.log('make_readable_rank');
  if (r < 11) {
    return r;
  } else if (r == 11) {
    return "J";
...
function reset_player_statuses (type) {
  console.log('reset_player_statuses');
  for (var i = 0; i < players.length; i++) {
    if (type == 0) {
      players[i].status = "";
...
function get_next_player_position (i, delta) {
  console.log('get_next_player_position');
  var j = 0;
  var step = 1;
  if (delta < 0) step = -1;
